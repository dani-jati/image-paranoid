...
def find_line_y(mask, min_contour_area=200):
...
def main():
    source_folder = "facial_thirds_imgs"
    target_folder = "is_narrow_imgs"
    ...
    for filename in sorted(os.listdir(source_folder)):
        ...
        h, w = img.shape[:2]
        ys = {color: find_line_y(build_mask(hsv, ranges)) for color, ranges in COLOR_RANGES.items()}
        print(f"\n🖼️ {filename}")
        ...
        annotated = img.copy()
        cv2.line(annotated, (0, ys["red"]), (w, ys["red"]), (0, 0, 255), 2)
        cv2.line(annotated, (0, ys["yellow"]), (w, ys["yellow"]), (0, 255, 255), 2)
        cv2.line(annotated, (0, ys["blue"]), (w, ys["blue"]), (255, 0, 0), 2)
        ...
        cv2.imwrite(os.path.join(target_folder, filename), annotated)
