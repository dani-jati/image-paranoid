"""
1. detect if left and right neck controus are upright (if not done yet, run script neck straighness).
2. If yes, measure shoulder distance and tilt angle against neck contours.
3. If not, measure shoulder tilt angle against nose line.


# 1. Find the meeting point of the two shoulder lines
meeting_point = find_intersection(p1, p2, p3, p4)

if meeting_point:
    # Draw the meeting point
    cv2.circle(img, meeting_point, 8, (255, 255, 255), -1)
    cv2.putText(img, "Meeting Point", (meeting_point[0] + 10, meeting_point[1]), 
                cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 255, 255), 1)

    # 2. Calculate distance to Nose Line
    # Use the nose line points stored in self during load_image
    nose_p1 = (int(self.nose_tip_x), int(self.nose_tip_y)) # or use your line_p1/p2
    nose_p2 = (int(self.nose_root_x), int(self.nose_root_y))
    
    dist_to_nose = point_to_line_distance(meeting_point, nose_p1, nose_p2)
    
    # Log or display the result
    self.add_log(f"📏 Distance from intersection to nose line: {dist_to_nose:.2f} px")
    cv2.putText(img, f"Dist to Nose: {dist_to_nose:.1f}px", (legend_x, legend_y + 155),
