# This script is to determine whether:
# 1. the neck outline is straight or curved.
# 2. if straight, whether it is upright vertical or slanted.
# 3. if curved, whether it is concave or convex.

# the idea is to click 5 points along the neck outline, 
# then fit a 4th degree polynomial to the points,
# then analyze the polynomial to determine the neck shape.

# If the shape is straight, 
# we can use the slope of the line to determine 
# if it is upright vertical or slanted
# relative to midline of the nose (radix and nose point landmarks).
# If the shape is curved, we can use the second derivative
# to determine if it is concave, convex, or rounding.   

# the output will be a text file with the results in format (e.g. JSON)
# that can be read by another module.
...
neck_outline_input = os.path.join(script_dir, "../images/input_images/neck_outline")
path = neck_outline_input

if os.path.isdir(cropper_output) and not os.path.exists(neck_outline_input):

    if os.name == 'posix': #linux or mac
        os.symlink(cropper_output, neck_outline_input) #linux
    elif os.name == 'nt': #windows    os.symlink(cropper_output, neck_outline_input)
        os.symlink(cropper_output, neck_outline_input, target_is_directory=True) #windows    

    print(f"🔗 Symlink created: {neck_outline_input} → {cropper_output}")
elif os.path.islink(neck_outline_input):
    print(f"✅ Symlink already exists: {neck_outline_input} → {os.readlink(neck_outline_input)}")
elif os.path.isdir(neck_outline_input):
    print(f"⚠️ Destination exists as a real folder: {midface_height_input} — not creating symlink.")
else:
...
print("Symlink points to:", os.readlink(neck_outline_input))

output_folders = {
    "linier_upright": os.path.join(script_dir, "../images/output_images/neck_outline/linier_upright"),
    "linier_slanted": os.path.join(script_dir, "../images/output_images/neck_outline/linier_slanted"),
    "concave": os.path.join(script_dir, "../images/output_images/neck_outline/concave"),
    "convex": os.path.join(script_dir, "../images/output_images/neck_outline/convex"),
}
for folder in output_folders.values():
    os.makedirs(folder, exist_ok=True)

progress_file = os.path.join(script_dir, "../progress/neck_outline.txt")
...
class Dashboard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Neck Outline Dashboard")
        self.resize(1200, 800)

        # Log
        self.log_file = open(os.path.join(script_dir, "../session_log/neck_outline.txt"), "a", encoding="utf-8")
        ...
        # State
        self.clicks = []
        self.files = [os.path.join(neck_outline_input, f) for f in sorted(os.listdir(neck_outline_input)) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
        self.index = 0
    ...
    def load_image(self, filename):
        ...
        preview = img.copy()
        clue = "Click the highest point on the left neck contour!"
    ...
    def on_click(self, event):
        ...
        if step == 1:
            self.add_log("✅ The highest point on the left neck contour recorded.")

            # instruction
            legend_x, legend_y = 0,0
            clue = "Click the lowest point on the left neck contour!"
            ...
            self.add_log("Drawing guide line from p1 to p2")
            p1,p2 = coords[0], coords[1]

            # guide line
            cv2.line(img_copy, (p1[0] - 40 , p1[1]), (p2[0] - 40 , p2[1]), (255,0,0), 3)

            cv2.circle(img_copy, (p1[0] - 40 , p1[1]), 5, (255, 0, 0), -1)
            cv2.circle(img_copy, (p2[0] - 40 , p2[1]), 5, (255, 0, 0), -1)
    ...
    def process_clicks(self):
        ...
        cv2.putText(img, "Green = neck outline path", (legend_x, legend_y+15),
