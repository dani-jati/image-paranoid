...
head_height_input = os.path.normpath(os.path.join(script_dir, "..", "images", "input_images", "head_height"))
path = head_height_input

if os.path.isdir(cropper_output) and not os.path.exists(head_height_input):

    if os.name == 'posix': #linux or mac
        os.symlink(cropper_output, head_height_input) #linux
    elif os.name == 'nt': #windows
        os.symlink(cropper_output, head_height_input, target_is_directory=True) #windows

    print(f"🔗 Symlink created: {head_height_input} → {cropper_output}")
elif os.path.islink(head_height_input):
    print(f"✅ Symlink already exists: {head_height_input} → {os.readlink(head_height_input)}")
elif os.path.isdir(head_height_input):
    print(f"⚠️ Destination exists as a real folder: {head_height_input} — not creating symlink.")
else:
...
progress_file = os.path.join(script_dir, "../progress/head_height.txt")
...
class Dashboard(QMainWindow):
    def __init__(self):
        ...
        # Log
        self.log_file = open(os.path.join(script_dir, "../session_log/head_height.txt"), "a", encoding="utf-8")
        ...
        # State
        self.clicks = []
        self.files = [os.path.join(head_height_input, f) for f in sorted(os.listdir(head_height_input)) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
        self.index = 0
