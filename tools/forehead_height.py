# This script is to measure forehead height relative to face height.
# Forehead height: distance from glabella to mid hair border.
# Face height: distance  from mid hair border to chin.
...
def classify_perspective(p1, p2):
    dx = abs(p1[0] - p2[0])
    dy = abs(p1[1] - p2[1])
    if dx < 30:  # forehead too close, probably aside
        return "Side"
...
class Dashboard(QMainWindow):
    def __init__(self):
        ...
        # Log
        self.log_file = open(os.path.join(script_dir, "../session_log/forehead_height.txt"), "a", encoding="utf-8")
        ...
        # State
        self.clicks = []
        self.files = [os.path.join(forehead_height_input, f) for f in sorted(os.listdir(forehead_height_input)) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
        self.index = 0
