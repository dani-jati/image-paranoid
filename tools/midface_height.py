...
output_folders = {
    "high": os.path.join(script_dir, "../images/output_images/midface_height/1.2_or_more"),
    "mid": os.path.join(script_dir, "../images/output_images/midface_height/1_to_1.2"),
    "low": os.path.join(script_dir, "../images/output_images/midface_height/0_to_1"),
}
...
midface_height_input = os.path.normpath(os.path.join(script_dir, "..", "images", "input_images", "midface_height_input"))
path = midface_height_input

if os.path.isdir(cropper_output) and not os.path.exists(midface_height_input):

    if os.name == 'posix': #linux or mac
        os.symlink(cropper_output, midface_height_input) #linux
    elif os.name == 'nt': #windows    os.symlink(cropper_output, midface_height_input)
        os.symlink(cropper_output, midface_height_input, target_is_directory=True) #windows    

    print(f"🔗 Symlink created: {midface_height_input} → {cropper_output}")
elif os.path.islink(midface_height_input):
    print(f"✅ Symlink already exists: {midface_height_input} → {os.readlink(midface_height_input)}")
elif os.path.isdir(midface_height_input):
...
class Dashboard(QMainWindow):
    def __init__(self):
        ...
        # Log
        self.log_file = open(os.path.join(script_dir, "../session_log/midface_height.txt"), "a", encoding="utf-8")
        ...
        # State
        self.clicks = []
        self.files = [os.path.join(midface_height_input, f) for f in sorted(os.listdir(midface_height_input)) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
        self.index = 0
