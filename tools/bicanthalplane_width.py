...
bicanthalplane_width_input = os.path.join(script_dir, "../images/input_images/bicanthalplane_width")

if os.path.isdir(cropper_output) and not os.path.exists(bicanthalplane_width_input):
    os.symlink(cropper_output, bicanthalplane_width_input)
    print(f"🔗 Symlink created: {bicanthalplane_width_input} → {cropper_output}")
elif os.path.islink(bicanthalplane_width_input):
    print(f"✅ Symlink already exists: {bicanthalplane_width_input} → {os.readlink(bicanthalplane_width_input)}")
elif os.path.isdir(bicanthalplane_width_input):
    print(f"⚠️ Destination exists as a real folder: {bicanthalplane_width_input} — not creating symlink.")
else:
    print(f"⚠️ Cropper output folder missing: {cropper_output}")


print("Symlink points to:", os.readlink(bicanthalplane_width_input))

output_folders = {
    "wide": os.path.join(script_dir, "../images/output_images/bicanthalplane_width/1.65_or_more"),
    "mid": os.path.join(script_dir, "../images/output_images/bicanthalplane_width/1.35_to_1.65"),
    "narrow": os.path.join(script_dir, "../images/output_images/bicanthalplane_width/0_to_1.35"),
}
for folder in output_folders.values():
    os.makedirs(folder, exist_ok=True)

progress_file = os.path.join(script_dir, "../progress/bicanthalplane_width.txt")
...
class Dashboard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Bicanthal Plane Width Dashboard")
        self.resize(1200, 800)

        # Log
        self.log_file = open(os.path.join(script_dir, "../session_log/bicanthalplane_width.txt"), "a", encoding="utf-8")
        ...
        # State
        self.clicks = []
        self.files = [os.path.join(bicanthalplane_width_input, f) for f in sorted(os.listdir(bicanthalplane_width_input)) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
        self.index = 0
    ...
    def process_clicks(self):
        ...
        p3_shifted = (p3[0], p3[1] + 20)
        p4_shifted = (p4[0], p4[1] + 20)
        ...
        cv2.circle(img, p3_shifted, 3, (0, 255, 255), -1)

        # Draw small circles at end points
        cv2.circle(img, p4_shifted, 3, (0, 255, 255), -1)
        ...
        for i in range(1, 3):  # division points at 1/3 ... 2/3
            ...
            cv2.circle(img, div, 3, (0, 255, 255), -1)

            # Add labels below the division points
            cv2.putText(img, f"{i}/3", (div[0] - 20, div[1] + 20),
        ...
        # Draw small circles at division points
        cv2.circle(img, div1, 0, (0, 255, 255), -1)
        cv2.circle(img, div2, 0, (0, 255, 255), -1)

        # Add labels below the division points
        cv2.putText(img, "1", (div1[0] - 40, div1[1] + 20),
            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
        cv2.putText(img, "2", (div2[0] - 40, div2[1] + 20),
            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
        """
